...
public class TableService {
    ...
    public boolean saveTable(Table table) {
        ...
        try {
            ...
            ps.setBoolean(1, table.isEnabled());
    ...
    public boolean reserveTable(int id, String NameUSer) {
        boolean condition = false;
        String query = "UPDATE tables SET enabled=?, nameUser=? WHERE id=?";
        ...
        try {
            ...
            ps.setString(2, NameUSer); // Заполняем имя пользователя в первый параметр запроса
            ps.setInt(3, id); // Заполняем id стола во второй параметр запроса
            int result = ps.executeUpdate();
